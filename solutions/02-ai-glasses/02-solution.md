...
AI 眼镜品类当前提供 **两类接入方式**，可按需选用：

1. **多模态交互开发套件（套件方案）** — 端到端打包，开箱即用
2. **裸模型直调（自拼方案）** — 自行拼接 ASR + LLM + TTS 链路

| 维度 | 多模态交互开发套件 | 裸模型直调 |
|---|---|---|
| 集成粒度 | 端到端打包（ASR+LLM+TTS+端侧SDK） | 自己拼 ASR → LLM → TTS |
| 端侧能力 | 内置 VAD / 回声消除 / 语音唤醒 | 自己实现 |
| 对话模式 | 全双工流式可打断 | 请求-响应 |
| 配置方式 | 可视化无代码（模型/提示词/知识库/Agent/插件/指令） | 写代码 |
| 系统覆盖 | Android / iOS / Linux / RTOS | 云端 / 移动 |
| 场景模板 | 智能眼镜 / 学习机 / 机器人 / 玩具 / 音箱 | 无 |
| 适用场景 | 快速跑通端侧 demo、需要全双工、目标含 RTOS | 已有成熟端侧框架、强定制需求 |
