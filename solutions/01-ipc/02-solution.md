...
- 仅需摘要（Caption）能力、或预算敏感 → 优先百炼方案（本文第一章）
- 需要完整检索 + 告警 + 每日总结 + 成片能力 → 走 OSS 方案（本文第二~十章）
- 两者可组合：实时由百炼生成 Caption → 写入 OSS → 被 AI 内容感知索引 → 支持后续检索

---

## 一、摄像头画面洞察 / 视频摘要 Caption（百炼方案）
...
### 1.7 与 OSS AI 内容感知的关系

两套方案的定位差异和组合方式详见本文档开头「方案总览」对比表。
