...
| 方案 | 体感延迟（轻度） | 体感延迟（搜索） | 联网搜索 | 结论 |
|---|---:|---:|---|---|
| 方案 1 · 阻塞 | 5502 ms | 7416 ms | 支持 | 简单实现，体验最差 |
| 方案 2 · 流式 | 2473 ms | 2980 ms | 支持 | 体验最优的千问大模型串接方案 |
| 方案 3 · Omni (HTTP) | 1230 ms | — | 不支持 | 单模型端到端，体感快 |
| 方案 4 · 多模态套件 | 997 ms | **1660 ms** | 支持 | 平台托管，支持搜索+应用配置 |
| 方案 3b · Omni Realtime (WS) | **330 ms** | — | 不支持 | 体感最快，支持语义打断 |

**选型建议**：

- 追求极致体感 + 语义打断 → 方案 3b（WebSocket Realtime，≈350ms）
- 需要联网搜索 + 平台托管 + 应用级能力 → 方案 4（轻度 ≈1s，搜索 ≈1.7s）
- 需要联网搜索 + 自主拼接灵活性 → 方案 2 流式（≈3s）
