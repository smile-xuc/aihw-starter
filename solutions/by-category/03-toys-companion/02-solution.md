...
> ⚖️ **合规提醒**：《人工智能拟人化互动服务管理暂行办法》（2026-07-15 施行）第十四条规定不得向未成年人提供**虚拟亲属、虚拟伴侣等虚拟亲密关系服务**。「爸妈声音陪伴」应定位为**监护人发起、复刻真实亲属声音的内容播报工具**（讲故事、读绘本），并保留「不冒充真人家长」的提示词约束（见 3.1 节），与「虚构虚拟爸妈人设并与孩子持续情感互动」严格区分；产品措辞和交互设计上线前建议做法务评估。
...
在声音之外，亲情包可叠加「形象」维度。两条技术路径：

| 路径 | 做法 | 现状 |
|---|---|---|
| 实时数字人 | 端侧/云端实时渲染驱动的数字人 | 成本高，本品类几乎无采用 |
| 照片 → 动态形象（推荐） | 用户上传照片 → qwen-image-3.0-pro 等图像模型生成复刻数字形象照 → 基于 HappyHorse / Wan 图生视频，生成数秒动态形象 | 成本低，具备独立收费点 |

推荐路径的产品要点：

- **离线一次性生成，非实时渲染**：形象照 + 数秒动态视频均为异步任务，生成后重复播放，无持续推理成本
- **独立收费点**：可收取一次性形象复刻费，与声音克隆打包为「亲情形象包」（声音 + 形象一次复刻）
- **展示位**：带屏设备（陪伴娃娃、伴学机）直接播放；无屏玩具可在家长小程序侧展示
- **合规**：肖像复刻须获得被复刻人书面授权；涉及儿童照片时遵守未成年人个人信息保护规定

生成链路：

```
家长上传照片 → qwen-image-3.0-pro（形象照复刻/风格化）
            → HappyHorse / Wan 图生视频（数秒动态形象）
            → 业务后端存储，绑定到角色（role_id → voice + avatar）
```
...
> ⚖️ **合规边界**：按《人工智能拟人化互动服务管理暂行办法》第十条，不得将「替代社会交往、控制用户心理、诱导沉迷依赖」作为服务目标，且提供者须具备过度依赖风险预警、情感边界引导、心理健康保护能力。本节的长记忆与主动陪伴设计应在此边界内实施，并同步落实 AI 身份标识与超 2 小时使用提醒（第十八条）。
