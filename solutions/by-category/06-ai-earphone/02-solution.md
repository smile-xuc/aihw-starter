...
3. **ASR + 摘要转写方案** — 适合录音转写 / 会议纪要场景（详见 [录音卡方案](../07-recorder/02-solution.md)）
...
- 做转写耳机 / 会议纪要 → **ASR + 摘要**（参考 [录音卡方案](../07-recorder/02-solution.md)）
- 产品同时需要翻译 + 对话 → 组合部署，按场景切换链路

以下章节重点介绍**端到端 S2S 翻译方案**的接入细节。三段拼接对话方案可参考 [AI 眼镜方案](../02-ai-glasses/02-solution.md)（套件即包含 ASR+LLM+TTS 全链路），ASR + 摘要转写方案参考 [录音卡方案](../07-recorder/02-solution.md)。
