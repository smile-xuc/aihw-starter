...
| Qwen-Omni Realtime（qwen3.5-omni-flash-realtime，WebSocket 双工） | 待国内网络补测 | 依不同模型而定，可扩容 | 支持 | [method3_omni_realtime](../benchmark/scripts/method3_omni_realtime_sdk.py) |

> ⚠️ 延迟数据因网络环境会略有不同，以上为特定测试环境下的参考值，具体测试条件见下方说明。
> `qwen3.5-omni-flash-realtime` 的 WebSocket 协议需要国内网络直连，海外出口会因中间代理导致连接中断。
