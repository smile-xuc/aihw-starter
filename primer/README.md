...
| 04 | [Token 与计费](./04-token-billing.md) | 一次对话到底花多少钱？输入 / 输出 / 缓存 / 音频 token 怎么算？为什么「同传一小时」和「聊五句」不是同一套账？ |
| 05 | [蒸馏](./05-distillation.md) | 蒸馏和量化有何不同？何时需要自研 / 采购端侧小模型？和端模型授权怎么一起看？ |
| 06 | [端云协同基础](./06-edge-cloud.md) | 端侧该干什么、云端该干什么？延迟 / 隐私 / 成本 / 能力怎么切？ |
| 07 | [上下文与记忆](./07-context-memory.md) | context window 和「长记忆」是不是一回事？怎样用最少成本做出「它还记得我」？ |
